...
var WeatherView = Backbone.View.extend({
    ...
    handleClick: function () {
        console.log('button')
        forecastModel.url = urlComposerForecast(weatherItem.get('lat'), weatherItem.get('long'))
        forecastList.url = urlComposerForecast(weatherItem.get('lat'), weatherItem.get('long'))
    ...
    forecastView: function () {
        ...
        forecastList.each(function(e,index){
            that.$el.append('<div>'+(index+1)+' day from now - ' + e.get('weather')[0].main +'</div>')
        })
    },
    my_template: _.template("<div><strong><%= res.name %></strong> (<%= (res.main.temp - 273).toFixed(2) + ' C' %>) - <%= 'wind: ' + res.wind.speed %></div><div><%= res.weather[0].main %><br><%= res.weather[0].description %></div><button> 3 Days Forecast </button>"),
    render: function () {
        console.log('here')
